...
const std::vector<int> PlayScene::code = {
    ...
    ALLEGRO_KEY_B, ALLEGRO_KEY_A, 216 /*ALLEGRO_KEYMOD_SHIFT*/, ALLEGRO_KEY_ENTER
};
...
void PlayScene::OnMouseUp(int button, int mx, int my) {
    ...
    const int y = my / BlockSize;
    if (button & 1) {
        if (mapState[y][x] != TILE_OCCUPIED || !preview->GetPrice()) {
            if (!preview) {
                return;
            }
            if (!preview->GetPrice()) {
...
void PlayScene::OnKeyDown(int keyCode) {
    ...
    if (keyCode == ALLEGRO_KEY_TAB) {
        ...
        } else if (keyStrokes.size() == code.size()) {
            int idx = 0;
            int spawn = 1;
    ...
    if (keyCode == ALLEGRO_KEY_Q) {
        ...
        // Hotkey for LaserTurret.
        UIBtnClicked(3);
    } else if (keyCode == ALLEGRO_KEY_ESCAPE && preview) {
        UIGroup->RemoveObject(preview->GetObjectIterator());
        preview = nullptr;
...
void PlayScene::ReadMap() {
    ...
    while (fin >> c) {
        ...
        case '\n':
        case '\r':
            if (static_cast<int>(mapData.size()) / MapWidth != 0) throw std::ios_base::failure("Map data is corrupted.");
            break;
        default:
    ...
    mapPrice = std::vector<std::vector<int>>(MapHeight, std::vector<int>(MapWidth));
    for (int i = 0; i < MapHeight; i++) {
        for (int j = 0; j < MapWidth; j++) {
...
void PlayScene::ConstructUI() {
    ...
    btn = new TurretButton(
            "play/floor.png", "play/dirt.png",
            Engine::Sprite("play/tool-base.png", 1294, 136 + (1370 - 1294), 0, 0, 0, 0),
            Engine::Sprite("play/shovel.png", 1294, 136 - 8 + (1370 - 1294), 0, 0, 0, 0), 1294, 136 + (1370 - 1294),
            Shovel::Price);
...
bool PlayScene::CheckSpaceOccupied(int x, int y) {
    if (x < 0 || x >= MapWidth || y < 0 || y >= MapHeight) {
        return false;
